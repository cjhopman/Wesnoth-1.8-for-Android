/* $Id: video.cpp 48450 2011-02-08 20:55:18Z mordante $ */
/*
...
#ifdef ANDROID
std::pair<int, int> CVideo::native_resolution(int flags) {
	SDL_Rect** modes = SDL_ListModes(NULL, flags);
	if (!modes) {
		ERR_DP << "No video modes supported ?!?!?" << std::endl;
		throw "error";
	}
	std::pair<int, int> res = std::make_pair(modes[0]->w, modes[0]->h);
	if (res.first < 640 || res.second < 480) {
		ERR_DP << "Max resolution is too tiny!" << std::endl;
		throw "error";
	}
	return res;
}
...
void CVideo::flip()
{
    ...
    unsigned time_ = SDL_GetTicks();
	if(update_all || time_ - last_update_ > 500) {
